          ...
          Row(
            ...
            children: [
              IconButton(
                ...
                icon: Icon(Icons.chevron_left_rounded, color: fy.colors.foreground, size: 28),
              ...
              IconButton(
                ...
                icon: Icon(Icons.chevron_right_rounded, color: fy.colors.foreground, size: 28),
                style: IconButton.styleFrom(
                  backgroundColor: fy.colors.muted.withOpacity(0.3),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                ),
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (index < firstDayOfMonth) {
          ...
          date = DateTime(_focusedDate.year, _focusedDate.month - 1, daysInPrevMonth - firstDayOfMonth + index + 1);
        } else if (index >= firstDayOfMonth + daysInMonth) {
          isCurrentMonth = false;
          date = DateTime(_focusedDate.year, _focusedDate.month + 1, index - (firstDayOfMonth + daysInMonth) + 1);
        } else {
          date = DateTime(_focusedDate.year, _focusedDate.month, index - firstDayOfMonth + 1);
        }
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: isToday && !isSelected
                  ? Border.all(color: fy.colors.primary.withOpacity(0.5), width: 1.5)
                  : null,
            ),
            child: Center(
              child: FyarnText(
                ...
                color: isSelected
                    ? fy.colors.onPrimary
                    : (isCurrentMonth ? fy.colors.foreground : fy.colors.mutedForeground.withOpacity(0.5)),
              ),
            ),
          ),
        );
