        ...
        children: [
          FyarnButton(
            ...
            child: Icon(
              Icons.chevron_left, 
              size: 18, 
              color: currentPage > 1 ? fy.colors.foreground : fy.colors.mutedForeground
            ),
          ),
          ...
          FyarnButton(
            ...
            child: Icon(
              Icons.chevron_right, 
              size: 18, 
              color: currentPage < totalPages ? fy.colors.foreground : fy.colors.mutedForeground
            ),
          ),
        ],
      ),
  ...
  List<Widget> _buildPageItems(FyarnTokens fy) {
    ...
    if (totalPages <= threshold) {
      for (int i = 1; i <= totalPages; i++) {
        items.add(_PageButton(page: i, isSelected: i == currentPage, onTap: () => onPageChanged(i), fy: fy));
      }
    } else {
      items.add(_PageButton(page: 1, isSelected: 1 == currentPage, onTap: () => onPageChanged(1), fy: fy));
      ...
      items.add(_PageButton(page: totalPages, isSelected: totalPages == currentPage, onTap: () => onPageChanged(totalPages), fy: fy));
    }
...
class _PageButton extends StatelessWidget {
  ...
  const _PageButton({required this.page, required this.isSelected, required this.onTap, required this.fy});
