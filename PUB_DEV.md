...
### 3. Setup the Engine
Wrap your `MaterialApp` with the `FyarnTheme`:

```dart
import 'package:your_app/components/core/theme/fyarn_theme.dart';
import 'package:your_app/components/core/theme/presets/zinc_colors.dart';

void main() {
  runApp(
    FyarnTheme(
      colors: FyarnColorsZinc.light,
      darkColors: FyarnColorsZinc.dark,
      child: const MyApp(),
    ),
  );
}
